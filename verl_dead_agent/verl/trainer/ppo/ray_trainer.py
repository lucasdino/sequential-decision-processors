...
class RayPPOTrainer:
    ...
    def _validate(self):
        reward_tensor_lst = []
        data_source_lst = []
        env_name_lst = []  # Track env names for per-env metrics
        success_rate_dict = {}
        ...
        for test_data in self.val_dataloader:
            # For multi-env setups, we need to run the same batch through each environment type
            for env_idx in range(num_env_types):
                ...
                print(f"[validate] Running env {env_idx + 1}/{num_env_types}")
                ...
                # Collect env_name for per-environment metrics
                env_names_batch = test_batch.non_tensor_batch.get('env_name', None)
                if env_names_batch is not None:
                    env_name_lst.append(env_names_batch)
                else:
                    # Fallback: get current env name from val_envs
                    current_env = getattr(self.val_envs, 'current_env_name', None) or 'unknown'
                    env_name_lst.append(np.array([current_env] * reward_tensor.shape[0], dtype=object))
        ...
        # Concatenate env names if collected
        env_names = np.concatenate(env_name_lst, axis=0) if env_name_lst else None
        ...
        # Add per-environment metrics for wandb
        if env_names is not None:
            env_reward = {}
            for i in range(reward_tensor.shape[0]):
                env_name = str(env_names[i]) if env_names[i] is not None else 'unknown'
                if env_name not in env_reward:
                    env_reward[env_name] = []
                env_reward[env_name].append(reward_tensor[i].item())
            
            for env_name, rewards in env_reward.items():
                safe_key = env_name.replace("/", "_")
                metric_dict[f'val/{safe_key}/test_score'] = np.mean(rewards)
                metric_dict[f'val/{safe_key}/sample_count'] = len(rewards)
    ...
    def init_workers(self):
        ...
        if self.use_intermediary:
            ...
            print("Intermediary initiated in RayPPOTrainer")
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("gen", timing_raw):
                        ...
                        if self.use_intermediary:
                            print("regualr gen batch", gen_batch)
